...
import Settings, { DEFAULT_SYSTEM_PROMPT, DEFAULT_VISUAL_BIBLE_PROMPT, DEFAULT_VISUAL_BIBLE_EDIT_PROMPT, DEFAULT_VISUAL_REFERENCE_PROMPT, DEFAULT_REFERENCE_ANALYSIS_PROMPT, DEFAULT_REFERENCE_DETAIL_PROMPT } from "../models/Settings.js";
import {
  encryptData,
  decryptData,
  maskSecret,
...
router.post("/", ensureAuthenticated, async (req, res) => {
  try {
    const body = req.body;
    const promptFields = ["theme", "script", "cover", "audio", "timelineDavinci", "visualBiblePrompt", "visualBibleEditPrompt", "visualReferencePrompt", "referenceAnalysisPrompt", "referenceDetailPrompt"];
    const optionalPromptFields = ["visualBiblePrompt", "visualBibleEditPrompt", "visualReferencePrompt", "referenceAnalysisPrompt", "referenceDetailPrompt"];
    ...
    settings.prompts = Object.fromEntries(promptFields.map((key) => [
      key,
      optionalPromptFields.includes(key) && !Object.hasOwn(prompts, key)
        ? settings.prompts?.[key] ?? ({ visualBiblePrompt: DEFAULT_VISUAL_BIBLE_PROMPT, visualBibleEditPrompt: DEFAULT_VISUAL_BIBLE_EDIT_PROMPT, visualReferencePrompt: DEFAULT_VISUAL_REFERENCE_PROMPT, referenceAnalysisPrompt: DEFAULT_REFERENCE_ANALYSIS_PROMPT, referenceDetailPrompt: DEFAULT_REFERENCE_DETAIL_PROMPT }[key])
        : prompts[key],
    ]));
